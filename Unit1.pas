...
begin
  ...
  //если все в пор€дке (не выгрузилось), то сменим указатели
  data.Free;
  data:=tmp;
  //таким образом, если проект не загрузилс€, предыдущий не потер€етс€.
  //и попытка освободить несуществующий объект тоже не будет предприн€та
  make_connections;
end;
...
begin
  ...
  //если есть что показать - покажем
    ...
    str:=' оординаты (%.'+IntToStr(precx)+'g ; %.'+IntToStr(precy)+'g)';
...
//    state:=1;
//    StatusBar1.Panels[0].Text:='¬ырезать пр€моугольный участок';
  end;
end;
